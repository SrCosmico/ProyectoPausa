...
export const preguntasPSS4Data: PreguntaPSS4[] = [
  { id: "p1", enunciado: "En el último mes, ¿con qué frecuencia has sentido que no podías controlar las cosas importantes de tu vida?", esInversa: false },
  { id: "p2", enunciado: "En el último mes, ¿con qué frecuencia te has sentido seguro de tu capacidad para manejar tus problemas personales?", esInversa: true },
  { id: "p3", enunciado: "En el último mes, ¿con qué frecuencia has sentido que las cosas salían como tú querías?", esInversa: true },
  { id: "p4", enunciado: "En el último mes, ¿con qué frecuencia has sentido que las dificultades se acumulaban tanto que no podías superarlas?", esInversa: false },
];
...
export default function EvaluacionFlujoPage() {
  ...
  const cambiarPasoSeguro = (nuevoPaso: number) => {
    ...
    setTimeout(() => { setPasoActual(nuevoPaso); setAnimarCambio(false); }, 200);
  };
  ...
  const retrocederPaso = () => {
    if (pasoActual <= 1) cambiarPasoSeguro(0);
    else cambiarPasoSeguro(pasoActual - 1);
  };
  ...
  const calcularResultadoEstres = () => {
    ...
    preguntasPSS4Data.forEach((p) => {
      ...
      puntajeTotal += p.esInversa ? (4 - pts) : pts;
    });
  ...
  // ─── Nuevo handler: guarda en Supabase y luego navega ──────────────────
  const handleGuardarResultado = async () => {
    if (!infoResultado) return;
    setGuardando(true);
    setErrorGuardar(null);

    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error('No hay sesión activa. Inicia sesión e intenta de nuevo.');

      const { error } = await insertarResultadoEvaluacionEstres(
        user.id,
      ...
      console.error('Error al guardar resultado PSS-4:', err);
      setErrorGuardar(err instanceof Error ? err.message : 'Error inesperado. Intenta de nuevo.');
      setGuardando(false);
    }
  };
            ...
            <div className="flex flex-col h-full justify-between">
              <div>
                ...
                <div className="space-y-3">
                  {["Solo 4 preguntas", "Toma 1-2 minutos", "Resultados al instante", "100% confidencial"].map((text, idx) => (
              ...
              <div className="mt-8 text-center">
                <button onClick={() => cambiarPasoSeguro(1)} className="w-full bg-[#4A72A6] hover:bg-[#3B5E8C] text-white font-semibold py-4 rounded-2xl shadow-md text-sm">
                  Comenzar test
                </button>
                <p className="text-[10px] text-slate-400 font-medium mt-3">Tus respuestas son privadas y se usan solo para tu bienestar.</p>
              </div>
            </div>
          )}
          ...
          {pasoActual >= 1 && pasoActual <= 4 && (
            <div>
              ...
              <div className="w-full bg-slate-100 h-1.5 rounded-full mt-3 overflow-hidden">
                <div className="bg-purple-400 h-full rounded-full transition-all duration-300" style={{ width: `${(pasoActual / 4) * 100}%` }} />
              </div>
              <div className="mt-8 min-h-[80px]">
                <h3 className="text-base font-extrabold text-[#2A3B50] leading-relaxed">{preguntaActual.enunciado}</h3>
              </div>
              <div className="mt-6 space-y-3">
                {opcionesPSS4Data.map((opcion) => {
                  const seleccionado = respuestas[preguntaActual.id] === opcion.puntosBase;
                  return (
                    <button key={opcion.texto} onClick={() => seleccionarOpcion(opcion.puntosBase)}
                      className={`w-full flex items-center justify-between p-4 rounded-2xl border text-left transition-all text-xs font-bold ${seleccionado ? 'bg-blue-50/40 border-[#4A72A6] text-[#4A72A6]' : 'bg-white border-slate-100 text-slate-600 hover:bg-slate-50'}`}>
                      <div className="flex items-center gap-3">
                        <span className="text-base">
                          {opcion.puntosBase === 0 && "😊"}{opcion.puntosBase === 1 && "🙂"}
                          {opcion.puntosBase === 2 && "😐"}{opcion.puntosBase === 3 && "🙁"}
                          {opcion.puntosBase === 4 && "😩"}
                        </span>
                        <span>{opcion.texto}</span>
                      </div>
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${seleccionado ? 'border-[#4A72A6]' : 'border-slate-300'}`}>
              ...
              <div className="mt-8 text-center space-y-4">
                <button onClick={avanzarPregunta} disabled={respuestas[preguntaActual.id] === undefined}
                  className={`w-full font-semibold py-4 rounded-2xl transition-all text-sm ${respuestas[preguntaActual.id] !== undefined ? 'bg-[#4A72A6] text-white hover:bg-[#3B5E8C]' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}>
                  {pasoActual === 4 ? "Ver mis resultados" : "Siguiente"}
                </button>
                <button onClick={retrocederPaso} className="text-xs font-bold text-slate-500 hover:text-slate-700 flex items-center justify-center gap-1 mx-auto">
                  ← Volver
                </button>
              </div>
            </div>
            ...
            <div className="space-y-5 animate-fade-in">
              ...
              <div className={`p-5 rounded-3xl border flex items-center justify-between ${infoResultado.colorBg}`}>
                <div>
                  ...
                  <h3 className={`text-2xl font-extrabold mt-1 ${infoResultado.colorText}`}>{infoResultado.nivel}</h3>
                </div>
              ...
              <div className="bg-white border border-slate-100 rounded-2xl p-4">
                ...
                <div className="relative w-full h-3 bg-slate-100 rounded-full mt-3">
                  ...
                  <div className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-indigo-900 rounded-full border-2 border-white shadow transition-all duration-500"
              ...
              <div className="pt-2 space-y-2.5">
                {/* Botón principal ahora llama a Supabase antes de navegar */}
                <button
                  onClick={handleGuardarResultado}
                  disabled={guardando}
                  className="w-full bg-[#4A72A6] hover:bg-[#3B5E8C] disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-semibold py-4 rounded-2xl shadow-sm text-sm transition-all flex items-center justify-center gap-2"
                >
                  {guardando && (
                ...
                <button onClick={() => cambiarPasoSeguro(0)} className="w-full bg-white border border-slate-200 text-slate-500 hover:text-slate-700 font-semibold py-3.5 rounded-2xl text-xs">
                  Volver al inicio
                </button>
              </div>
            </div>
