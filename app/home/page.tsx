...
export const accesoRapidoData: AccesoRapido[] = [
  ...
  {
    id: "racha",
    titulo: "Racha con amigos",
    descripcion: "Cuídense juntos cada día",
    ruta: "/racha",
  },
  {
    id: "crisis",
...
const SALUDOS: Record<string, string> = {
  estres_academico:
    "Estamos aquí para ayudarte con el estrés académico 📚",

  sobrecarga_tareas:
    "Vamos a organizar tu carga de tareas juntos 📝",

  falta_tiempo:
    "Te ayudamos a encontrar tiempo para ti 🕒",

  problemas_personales:
    "Estamos aquí para acompañarte 💜",

  ansiedad:
    "Tu calma es nuestra prioridad 🧘",

  motivacion_baja:
    "Vamos a recuperar tu motivación juntos ✨",
};
...
function ajustarUmbralCrisis(frecuencia: string) {
  ...
  if (frecuencia === "todos_los_dias") {
    localStorage.setItem(
      "umbral_crisis_personalizado",
      "2.2"
    );
  } else if (frecuencia === "varias_semana") {
    localStorage.setItem(
      "umbral_crisis_personalizado",
      "2.0"
    );
...
export default function HomePage() {
  ...
  const [herramientas, setHerramientas] =
    useState<AccesoRapido[]>(accesoRapidoData);

  const [saludo, setSaludo] =
    useState("Nos alegra que estés aquí");

  const [estadoRacha, setEstadoRacha] =
    useState<EstadoRacha | null>(null);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (calcularEstadoRachaPareja) {
        try {
          setEstadoRacha(
            await calcularEstadoRachaPareja(user.id)
          );
        } catch {
      ...
      if (meta?.onboarding_completado) {
        ...
        const factor =
          meta.factores_impacto?.[0] ?? "";

        if (SALUDOS[factor]) {
          setSaludo(SALUDOS[factor]);
        }
      }
    };
