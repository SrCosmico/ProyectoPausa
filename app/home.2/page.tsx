...
export default function HomePage() {
  ...
  const [herramientas, setHerramientas] = useState<AccesoRapido[]>([]);
  const [saludo, setSaludo] = useState('');
  const [rachaActual] = useState(0);
  ...
  const datosHome = {
    usuario: { nombre: usuarioNombre, correo: '' },
    saludo,
    registroEmocional: {
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span className="text-sm font-bold text-orange-600">{rachaActual}</span>
                </button>
              ...
              {datosHome.accesoRapido.map((item) => {
                ...
                return (
                  <button
                    key={item.id}
                    onClick={() => {
                      if (tieneNavegacion) router.push(item.ruta);
                    }}
