...
function ModoCrisisContenido() {
  const router = useRouter();
  const searchParams = useSearchParams();

  // Antes esto estaba fijo en `false`/`0`, así que aunque el Monitoreo de
  // Bienestar mandara ?auto=true&promedio=X en la URL para avisar que el
  // usuario llegó aquí automáticamente por un promedio bajo, esta pantalla
  // lo ignoraba por completo. Ahora sí se leen los parámetros reales.
  const desencadenadoAutomaticamente = searchParams.get("auto") === "true";
  const promedioParam = searchParams.get("promedio");
  const promedioBienestar: number = promedioParam ? parseFloat(promedioParam) : 0;
