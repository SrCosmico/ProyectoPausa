...
import { crearActividadCronograma } from '@/lib/supabase/cronograma';
...
export default function CronogramaPage() {
  ...
  // --- ESTADOS DEL FORMULARIO "AGREGAR ACTIVIDAD" (conectado a Supabase) ---
  ...
  // --- GUARDAR ACTIVIDAD: inserta en Supabase con la hora ya convertida a 24h ---
  const handleGuardarActividad = async () => {
    ...
    try {
      ...
      const { error } = await crearActividadCronograma({
        ...
        hora_inicio: horaInicioActividad, // se convierte internamente de "07:00 a. m." -> "07:00:00"
      ...
      // Limpiar formulario y volver a la vista semanal
      ...
      setErrorActividad("Ocurrió un error inesperado al guardar la actividad.");
      setGuardandoActividad(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-0 sm:p-4 font-sans selection:bg-slate-200">
      ...
      <div className="w-full max-w-md h-screen sm:h-[850px] bg-white shadow-2xl flex flex-col justify-between relative sm:rounded-[40px] border border-gray-100 overflow-hidden pb-4">
        ...
        <div className="flex-1 overflow-y-auto bg-white custom-scrollbar">
          ...
          {/* PANTALLA 9: AGREGAR ACTIVIDAD (FORMULARIO CONECTADO A SUPABASE) */}
            ...
            <div className="p-6 space-y-4 animate-fadeIn">
              ...
              <div className="flex gap-2 justify-between">
                {tiposActividad.map((t) => {
                  ...
                  return (
                    <button
                      key={t.id}
                      onClick={() => setTipoNuevaActividad(t.id)}
              ...
              <div className="space-y-3 pt-2">
                <div className="space-y-1">
                  ...
                  <input
                    type="text"
                    value={tituloNuevaActividad}
                    onChange={(e) => setTituloNuevaActividad(e.target.value)}
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="text"
                    value={ubicacionNuevaActividad}
                    onChange={(e) => setUbicacionNuevaActividad(e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none cursor-pointer appearance-none"
                    >
                      {opcionesHoras.map((hora) => (
                        <option key={hora} value={hora}>{hora}</option>
                      ))}
