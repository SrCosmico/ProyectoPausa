  ...
  return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-0 sm:p-4 font-sans selection:bg-blue-100 relative overflow-hidden">
      ...
      <motion.div 
        ...
        className="w-full max-w-md min-h-screen sm:min-h-[850px] sm:max-h-[900px] bg-white shadow-2xl overflow-hidden flex flex-col justify-between relative sm:rounded-[40px] border border-gray-100 z-10"
      >
        
        {/* ELEMENTOS PLASMÁTICOS DENTRO DEL CONTENEDOR */}
        <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 10, repeat: Infinity }} className="absolute -top-[10%] -left-[20%] w-80 h-80 rounded-full bg-purple-100/60 blur-[80px] -z-10" />
        <motion.div animate={{ scale: [1.1, 1, 1.1] }} transition={{ duration: 12, repeat: Infinity }} className="absolute top-[30%] -right-[20%] w-80 h-80 rounded-full bg-blue-100/60 blur-[80px] -z-10" />
        <motion.div animate={{ y: [0, -20, 0] }} transition={{ duration: 8, repeat: Infinity }} className="absolute -bottom-[5%] left-[10%] w-96 h-96 rounded-full bg-green-100/50 blur-[80px] -z-10" />
